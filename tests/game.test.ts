...
test('one-second rounds capture after one tick and retain the pace for later rounds', async () => {
  const { advanceRound } = await import('../src/game/roundFlow.ts')
  const store = makeGameStore()
  configure(store, { countdownSeconds: 1 })
  store.trigger.cameraReady()
  store.trigger.start({ opponentMove: 'scissors' })
  expect(store.getSnapshot().context.countdown).toBe(1)
  ...
  advanceRound(store, 1000)
  expect(store.getSnapshot().context.local.score).toBe(1)
  expect(store.getSnapshot().context.nextIn).toBe(3)
  for (let i = 0; i < 3; i++) store.trigger.tick()
  advanceRound(store, 4000)
  expect(store.getSnapshot().context.countdown).toBe(1)
  store.trigger.detected({ gesture: null, stable: false, at: 4900 })
  store.trigger.tick()
  advanceRound(store, 5000)
  expect(store.getSnapshot().context.round).toBe(2)
  expect(store.getSnapshot().context.local.score).toBe(1)
  expect(store.getSnapshot().context.notification?.kind).toBe('retry')
  store.trigger.reset()
  expect(store.getSnapshot().context.countdownSeconds).toBe(1)
})
